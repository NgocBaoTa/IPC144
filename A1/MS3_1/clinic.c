...
void viewAllAppointments(struct ClinicData* data) {
    ...
    for (i = 0; i < data->maxAppointments; i++) {
        ...
        for (j = 0; j < data->maxPatient && flag == 0; j++) {
            if (data->patients[j].patientNumber == data->appointments[i].patient.patientNumber) {
                strcpy(data->appointments[i].patient.name, data->patients[j].name);
                data->appointments[i].patient.phone = data->patients[j].phone;
                flag = 1;
            }
        }
    }
    ...
    while (i < data->maxAppointments) {
        if (data->appointments[i].patient.patientNumber > 0) {
            displayScheduleData(&data->appointments[i].patient, &data->appointments[i], 1);
        }
        i++;
    }
    printf("\n");
...
void viewAppointmentSchedule(struct ClinicData* data) {
    ...
    for (i = 0; i < data->maxAppointments; i++) {
        if (data->appointments[i].date.day == day && data->appointments[i].date.month == month &&
            ...
            displayScheduleData(&data->appointments[i].patient, &data->appointments[i], 0);
        }
    }
...
void addAppointment(struct Appointment* appointments, int maxAppointments,
    ...
    for (i = 0; i < maxAppointments && flag == 0; i++) {
        if (appointments[i].patient.patientNumber < 1) {
            index = i;
            flag = 1;
        }
    }
            ...
            do {
                ...
                for (i = 0; i < maxAppointments && flag == 0; i++) {
                    if (appointments[i].date.year == year && appointments[i].date.month == month &&
                        ...
                        if (appointments[i].patient.patientNumber == patientNum) {
                ...
                if (flag == 0) {
                    ...
                    do {
                        if (hour < START_HOUR || hour > END_HOUR || (hour == END_HOUR && min > END_MIN) ||
                            ...
                            appointments[index].patient.patientNumber = patientNum;
